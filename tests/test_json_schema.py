...
def test_constraint_names():
    assert 'multipleOf' in system.constraints
...
def test_format_toggle():
    ...
    system.ignore_all_formats = True
    assert s.validate("not a time")

    system.ignore_all_formats = False
    assert not s.validate("not a time")

    system.ignore_these_formats = ['dateTime']
    assert s.validate("not a time")

    system.ignore_these_formats = [] 
    assert not s.validate("not a time")
...
### Meta ###
def test_value():
    s = schema({'type': 'string', 'format': 'email', 'min_length': 8})
    ...
    s = schema({'type': 'array', 'items': {'type': 'string'}})
    ...
    assert s.value == schema(s.value).value
