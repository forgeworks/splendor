...
class SchemaFailure(ValidationError):
    def __init__(
        self, schema, constraints=None, message="could not validate the instance against the schema"
    ):
        self.schema = schema
...
class System:
    ...
    def register_constraint(self, constraint, primitives=None, name=None):
        ...
        if name in self.reserved_constraints:
            raise ValueError("Constraints cannot be any of: {!r}".format(self.reserved_constraints))
    ...
    def borrow_constraint(
        ...
        our_name=None,
        primitives=None,
        transform_to_theirs=identity,
        transform_to_ours=identity,
    ):
        ...
        self.add_constraint_transformation(
            our_name, their_name, from_system=self, to_system=other_system, fn=transform_to_theirs
        )
        self.add_constraint_transformation(
            their_name, our_name, from_system=other_system, to_system=self, fn=transform_to_ours
        )

    def add_constraint_transformation(
        self, from_name, to_name, fn=identity, from_system=None, to_system=None
    ):
    ...
    def transform_instance_via_constraint(
        self, instance, from_name, to_name, from_system=None, to_system=None
    ):
        """
    ...
    def transform_instance_via_primitive(
        self, instance, from_name, to_name, from_system=None, to_system=None
    ):
        from_node = self.get_node(from_system, "primitive", from_name)
        to_node = self.get_node(to_system, "primitive", to_name)
    ...
    def transform_instance_to_other_system(
        self, schema, instance, from_system=None, to_system=None, to_schema_name=None
    ):
    ...
    def transform_constraint(self, name, value, other_system):
        sig = (other_system, name)
        transformation = self.constraint_transformations.get(sig, None)
        if transformation:
            return transformation(name, value)
        return {other_system.name_inflection(name): value}
...
class Schema:
    ...
    def transform(self, system, other):
        ...
        for name, val in self.value.items():
            value.update(system.transform_constraint(name, val, other))
        return other.schema(value)
